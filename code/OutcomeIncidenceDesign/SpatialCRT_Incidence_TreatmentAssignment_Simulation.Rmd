...
* **Target of estimation:** The direct effect $\tau$ (coefficient on $Z$ in the SDM) is the estimand. The naive difference-in-means is biased for $\tau$ when spillover or spatial dependence is present.
* **Design vs outcome:** Treatment is assigned using a single "historical" incidence (first baseline draw). Outcome replicates use different baseline draws and errors to reflect sampling variation given that design.
...
# Load libraries for spatial modeling, data wrangling, and plotting
library(spdep)
library(spatialreg)
library(dplyr)
library(ggplot2)
...
```{r parameters}
# Toggle for Estimation Mode: "DIM" (Quick) or "MLE" (Thorough/Rigorous)
estimation_mode <- "MLE" # MLE or DIM

# Toggle for Design 6 (Center Hotspot / Concentric Rings)
include_design_6 <- FALSE 

# Base Parameters (Dynamically scaled based on estimation mode)
if (estimation_mode == "MLE") {
  # 25 design resamples x 10 outcome resamples = 250 iterations per scenario.
  # This provides a strong balance between robustness (testing multiple spatial 
  # allocations) and computational efficiency for the maximum likelihood estimator.
  n_designs_resamples <- 25   
  n_outcome_resamples <- 10  
} else {
  # Fast Difference-in-Means can easily handle 100,000 iterations per scenario
  n_designs_resamples <- 100  
  n_outcome_resamples <- 1000 
}

# Grid definitions: 10x10 grid yielding 100 clusters
grid_dim <- 10
N_clusters <- grid_dim^2
coords <- expand.grid(x = 1:grid_dim, y = 1:grid_dim)

# Spatial Parameters
rho_vals <- c(0.00, 0.01, 0.20, 0.50) # Spatial dependence (autocorrelation)
gamma_vals <- c(0.5, 0.6, 0.7, 0.8) # Spillover effect magnitude
spill_types <- c("control_only", "both") # Spillover target constraint
neighbor_types <- c("rook", "queen") # Contiguity definition

# SDM Parameters
true_tau <- 1.0   # True direct intervention effect
beta <- 1.0       # Effect of baseline incidence on the outcome
sigma <- 1.0      # Variance of the random error term

# Determine which designs to run based on the hotspot toggle
if(include_design_6) {
  design_ids <- 1:7
} else {
...
```{r spatial_weights}
# Generate basic neighbor lists identifying adjacent clusters
nb_rook <- cell2nb(grid_dim, grid_dim, type = "rook")
nb_queen <- cell2nb(grid_dim, grid_dim, type = "queen")

# Generate Row-Standardized Weight Matrices (Used in the Data Generating Process)
W_rook <- nb2mat(nb_rook, style = "W", zero.policy = TRUE)
W_queen <- nb2mat(nb_queen, style = "W", zero.policy = TRUE)

# Generate listw objects (Specifically formatted for lagsarlm() spatial regression)
...
get_designs <- function(design_id, n_resamples, N, incidence, nb_list) {
  # Initialize the matrix that will hold all treatment assignment vectors
  mat <- matrix(0, nrow = N, ncol = n_resamples)
  
  if (design_id == 1) {
    # Design 1: Checkerboard (Strict Block Stratification)
    cb1 <- (coords$x + coords$y) %% 2
    mat <- matrix(rep(cb1, n_resamples), ncol = n_resamples)
    
  } else if (design_id == 2) {
    # Design 2: High Incidence Focus (Treat only the worst-off clusters)
    med_val <- median(incidence)
    trt_assign <- ifelse(incidence > med_val, 1, 0)
    mat <- matrix(rep(trt_assign, n_resamples), ncol = n_resamples)
    
  } else if (design_id == 3) {
    # Design 3: Saturation Quadrants (Varying treatment density by region)
    ...
    # Design 4: Intervention Isolation (Force control buffers around treated units)
    for (i in 1:n_resamples) {
      z <- numeric(N)
      available <- 1:N
    ...
    # Design 5: 2x2 Blocking (Localized 1:1 randomization)
    ...
    # Design 7: Balanced Quartiles (1:1 randomization within incidence strata)
...
cat(sprintf("Starting SDM Simulation in %s Mode.\n", estimation_mode))
cat("Total scenarios:", total_scenarios, "| Iterations per scenario:", n_designs_resamples * n_outcome_resamples, "\n")
cat("--------------------------------------------------------------\n")

for (nb_type in neighbor_types) {
  
  # Set active matrix and listw
  W <- if(nb_type == "rook") W_rook else W_queen
  active_nb <- if(nb_type == "rook") nb_rook else nb_queen
  active_listw <- if(nb_type == "rook") listw_rook else listw_queen
  I <- diag(N_clusters)
    ...
    for (gamma in gamma_vals) {
      for (spill_type in spill_types) {
        for (d_id in design_ids) {
          ...
          # Capture start time for the current scenario
          scenario_start_time <- Sys.time()
          
          cat(sprintf("Processing %3d / %d | NB: %-5s | Rho: %.2f | Gam: %.2f | Spill: %-12s | Design: %d\n", 
                      list_index, total_scenarios, nb_type, rho, gamma, spill_type, d_id))
          flush.console()
          ...
          for (d_resample in 1:n_designs_resamples) {
            ...
            # 5. ESTIMATION STEP (Toggle between DIM and MLE)
            if (estimation_mode == "DIM") {
              ...
              # Loop through each simulated outcome universe to fit the spatial regression
              for (k in 1:n_outcome_resamples) {
                
                if (sum(Z) == 0 || sum(Z) == N_clusters) {
                  scenario_estimates[counter] <- NA
                } else {
                  df <- data.frame(Y = Y_sim[, k], Z = Z, Spill = spill_term, X = X_matrix[, k])
                  
                  # Fit the Spatial Durbin Model using Maximum Likelihood
                  # tryCatch handles scenarios where Z and Spillover are perfectly collinear
                  # suppressWarnings() is used to hide the "Aliased variables found" warning generated by collinearity.
                  fit <- tryCatch({
                    suppressWarnings(spatialreg::lagsarlm(Y ~ Z + Spill + X, data = df, listw = active_listw, quiet = TRUE))
                  }, error = function(e) NULL)
                  
                  # Extract the Z coefficient if the model successfully converged
                  if (!is.null(fit) && "Z" %in% names(coef(fit))) {
                    scenario_estimates[counter] <- coef(fit)["Z"]
                  } else {
                    scenario_estimates[counter] <- NA
                  }
                }
          ...
          # Compute failure rates (from multicollinearity) and aggregate results
          valid_estimates <- na.omit(scenario_estimates)
          fail_rate <- 1 - (length(valid_estimates) / length(scenario_estimates))
          ...
          if(length(valid_estimates) > 0) {
            ...
            # Handle the case where the design completely failed (100% collinearity)
            results_list[[list_index]] <- data.frame(
              Neighbor_Type = nb_type, Design = paste("Design", d_id),
              Rho = rho, Gamma = gamma, Spillover_Type = spill_type,
              Mean_Estimate = NA, Bias = NA, SD = NA, MSE = NA, Fail_Rate = fail_rate
            )
          }
          
          # Calculate and print elapsed time for the scenario
          ...
          cat(sprintf("   -> Scenario %d Elapsed Time: %d min %.1f sec\n", list_index, mins, secs))
          flush.console()
...
mse_rho_summary <- granular_results %>%
  ...
  mutate(text_color = ifelse(Avg_MSE > quantile(Avg_MSE, 0.75, na.rm=TRUE), "black", "white"))
...
comprehensive_table <- granular_results %>%
  ...
  dplyr::select(Design, Neighbor_Type, Spillover_Type, Rho, Gamma, Bias, SD, MSE)
...
plot_master_comparison <- ggplot(comparison_data, aes(x = Design, y = MSE, fill = as.factor(Gamma))) +
  ...
  facet_grid(
    ...
    scales = "free_y", 
    labeller = labeller(Spillover_Type = c("both" = "Spillover: Both", "control_only" = "Spillover: Control"))
  ) +
...
```{r}
# ==============================================================================
# Save R Environment for Future Analysis (MLE or DIM)
# ==============================================================================
# save the current R environment specific for when estimation is performed with "MLE" vs. "DIM". This allows us to keep the large MLE results without overwriting them when we run the faster DIM version for quick checks and visualizations.
save.image(file = paste0("SpatialCRT_Incidence_Simulation_", estimation_mode, "_Results.RData"))

# load the saved environment based on specification of MLE vs. DIM estimation mode (create an object to select whether to load MLE or DIM results for future analysis and visualization)
# estimation_mode_to_load <- "MLE" # Change to "DIM" to load the faster
# load(paste0("SpatialCRT_Incidence_Simulation_", estimation_mode_to_load, "_Results.RData"))
```
