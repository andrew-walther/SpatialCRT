...
if (estimation_mode == "MLE") {
  ...
  n_outcome_resamples <- 40   # 1,000 total iterations per scenario (computationally feasible for MLE)
} else {
  n_designs_resamples <- 100  
  n_outcome_resamples <- 1000 # 100,000 total iterations per scenario (fast for DIM)
}
...
# Spatial Parameters
rho_vals <- c(0.00, 0.01, 0.20, 0.50) # spatial dependence
gamma_vals <- c(0.5, 0.6, 0.7, 0.8) # spillover effect
spill_types <- c("control_only", "both") # spillover constraint (neighbors that are just control or either)
neighbor_types <- c("rook", "queen") # neighbor definition: just adjacent (rook) or diagonal as well (queen)
...
# Determine which designs to run (toggle on off to include "hot spot" design)
if(include_design_6) {
  design_ids <- 1:7
} else {
...
# Generate listw objects for lagsarlm() spatial regression
            ...
            if (estimation_mode == "DIM") {
              ...
              for (k in 1:n_outcome_resamples) {
                ...
                if (sum(Z) == 0 || sum(Z) == N_clusters) {
                  ...
                  fit <- tryCatch({
                    spatialreg::lagsarlm(Y ~ Z + Spill + X, data = df, listw = active_listw, quiet = TRUE)
...
# Exclude failed designs from visualization if any exist
plot_data <- granular_results %>% filter(!is.na(MSE))

# PLOT 1: Overall Design Robustness by Neighbor Type
plot_mse_overall <- ggplot(plot_data, aes(x = reorder(Design, MSE, FUN = median), y = MSE, fill = Design)) +
...
summary_by_design <- granular_results %>%
  ...
  summarise(
    ...
    Avg_Fail_Rate = mean(Fail_Rate, na.rm = TRUE)
  ) %>%
  arrange(Avg_MSE) # Sort best (lowest MSE) to worst

datatable(
  summary_by_design,
  caption = "Table 1: Overall Average Bias, SD, MSE, and Failure Rate by Design",
  rownames = FALSE,
  options = list(dom = 't') # Simple table without search bars for the summary
) %>%
  formatRound(columns = c("Avg_Bias", "Avg_SD", "Avg_MSE", "Avg_Fail_Rate"), digits = 4)
...
comprehensive_table <- granular_results %>%
  ...
  dplyr::select(Design, Neighbor_Type, Spillover_Type, Rho, Gamma, Bias, SD, MSE, Fail_Rate)
...
datatable(
  ...
  formatRound(columns = c("Bias", "SD", "MSE", "Fail_Rate"), digits = 4)
```
...
unique_designs <- sort(unique(plot_data$Design))

for (current_design in unique_designs) {
  
  design_data <- plot_data %>% filter(Design == current_design)
...
comparison_data <- plot_data %>% filter(Neighbor_Type == "queen")
