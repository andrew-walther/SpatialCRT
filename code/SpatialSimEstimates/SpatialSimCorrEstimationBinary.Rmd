...
for(i in 1:N){
  tic() # start timer for each iteration
  ### 1) Data Simulation ###
  ...
  dlist <- dnearneigh(coordinates, d1 = 0, d2 = 100, longlat = FALSE)
  W <- nb2listw(dlist, style = "W")
  # Compute the spatially correlated response using indicator-based weights
  Wy <- lag.listw(W, points$response)
  points$response_binary_IndCorr <- points$response + rho * Wy + epsilon # use "response" from (3A)
  ####################################################################################################
  ...
  Max_Neighbor_Dist <- 100 # Define the maximum distance for neighbors
  W_matrix <- ifelse(distance_matrix <= Max_Neighbor_Dist & distance_matrix > 0, 1 / distance_matrix, 0) # inverse distance decay weights
  W_matrix <- W_matrix / rowSums(W_matrix, na.rm = TRUE) # normalize weights
  W_matrix[is.na(W_matrix)] <- 0 # set potential NaNs to 0
  W <- mat2listw(W_matrix, style = "W", row.names = rownames(points)) # convert weights matrix to listw object
  # Compute spatially correlated response using the distance-based weights
  Wy <- lag.listw(W, points$response)
  points$response_binary_DistCorr <- points$response + rho * Wy + epsilon
  ####################################################################################################
  ...
  dlist <- dnearneigh(coordinates, d1 = 0, d2 = 100, longlat = FALSE) # d1 = min neighbor dist, d2 = max neighbor dist
  ...
  model_ind <- lagsarlm(response_binary ~ intervention_zone + spillover_zone, data = points, listw = W_ind)
  ...
  W_matrix <- ifelse(distance_matrix <= Max_Neighbor_Dist & distance_matrix > 0, 1 / distance_matrix, 0) # linear decay w/ increasing distance
  ...
  # Fit a Spatial Lag Model (SLM) w/ intervention effect, spillover effect, & spatial weights matrix (adapt for distance vs. indicator)
  model_dist <- lagsarlm(response_binary_IndCorr ~ intervention_zone + spillover_zone, data = points, listw = W_dist)
  ...
  toc(log = TRUE) # End timer for each iteration & and print the elapsed time
  cat(sprintf("Iteration %d: %s seconds\n", i, attr(tic("last"), "toc")["elapsed"]))
}
```
