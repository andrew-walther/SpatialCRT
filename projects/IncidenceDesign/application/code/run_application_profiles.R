...
if (basename(code_dir) == "code" && basename(dirname(code_dir)) == "application") {
  application_dir <- normalizePath(dirname(code_dir), mustWork = TRUE)
} else if (basename(code_dir) == "application") {
  application_dir <- normalizePath(code_dir, mustWork = TRUE)
  code_dir <- file.path(application_dir, "code")
} else {
  application_dir <- file.path(code_dir, "application")
  code_dir <- file.path(application_dir, "code")
}
