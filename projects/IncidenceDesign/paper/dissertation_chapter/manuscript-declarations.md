...
CTJ's Declarations (conflicting interests, funding) were added at step 7
(2026-09-25), verbatim from `paper/ctj_manuscript/CTJ_Manuscript.tex`
(lines 164–168 as of commit 3730dfc); the section headings are CTJ's own.

This file is copied verbatim to bios-dissertation
(`prelim/project-proposals/project2-incidence/manuscript-declarations.md`) by
`tools/sync_to_prelim.sh`; edit it here, not there.
...
## Author block (standalone render only)

The standalone `Dissertation_Chapter.qmd` prints this author block; the
bios-dissertation copy drops it (the prelim's front matter names the author),
so it is preserved here verbatim from the chapter's YAML, LaTeX markup
cleaned up only.

Andrew Walther^1^, Ross Joseph Simpson, Jr.^2^, Ashkan Habib^1^, and Feng-Chang Lin^1^
