...
SUDDEN-derived North Carolina county dataset is finalized. That dataset
(described in the main text's Application section) gives county-level sudden
unexpected death incidence among working-age adults (aged 18--64) for
2018--2021. It was derived from North Carolina death certificates in Habib's
unpublished master's paper (Department of Biostatistics, University of North
Carolina at Chapel Hill, 2026), and we aggregate it to the 58 service-area
clusters used here.

Figures~\ref{fig:si-cc-service-areas}--\ref{fig:si-kmeans-regions} show the
application geography itself: the 58 community-college service-area clusters
used as the treatment-assignment units, the synthetic placeholder incidence
surface used to inform incidence-aware designs, and the population-balanced
$k$-means regions used by the saturation-based designs in place of the grid
study's literal $2\times2$ quadrants.

\begin{figure}[h]
\centering
\includegraphics[width=0.75\linewidth]{figures/application_maps/community_college_service_area_clusters.png}
\caption{North Carolina Community College service-area clusters, numbered by
primary serving college. This 58-cluster irregular geography replaces the
grid study's regular $10\times10$ lattice for the application illustration.}
\label{fig:si-cc-service-areas}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.7\linewidth]{figures/application_maps/synthetic_incidence_map.png}
\caption{Synthetic placeholder SUD incidence surface (Poisson SAR process)
used to inform incidence-aware designs for this illustration. This is not
real surveillance data; see the placeholder caveat above.}
\label{fig:si-synthetic-incidence}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\linewidth]{figures/application_maps/kmeans_regions_map.png}
\caption{Population-balanced $k$-means regions used by Saturation Regions and
Incidence-Guided Saturation Regions in place of the grid study's literal
$2\times2$ quadrants (see the design-relabeling note below).}
\label{fig:si-kmeans-regions}
\end{figure}
