...
We now consider a series of spatial grid setups to address efficient estimation of the intervention effect. These spatial grids have dimensions denoted as (row $\times$ column) and we seek to apply a balanced distribution between intervention and control cells where possible. When this is not possible, we defer to the control group being the majority class as mentioned previously.

\subsubsection{$2\times4$ Grid}
...
\end{equation}
\noindent where $j$ indexes clusters and $w_{ij} = 0$ whenever clusters $i$ and $j$ are not Rook neighbors, so the sum reduces naturally to neighboring clusters only. Full definitions of indicator-based and distance-based weight construction, including row-normalization formulas, are provided in Appendix~\ref{appendix:SpatialWeights}.

\subsection{Estimation for SAR Model}

% SUPP-2: Full likelihood/log-likelihood moved to Appendix B
Maximum likelihood estimation is used to estimate the cluster-level spatial lag model in Equation~(\ref{Equation:SARmodel})\cite{ord_estimation_1975}\cite{bivand_comparing_2015}\cite{bivand_computing_2013}. The full likelihood and log-likelihood expressions, along with details of the MLE derivation, are provided in Appendix~\ref{appendix:SARLikelihood}. In practice, estimates are obtained via the \texttt{lagsarlm()} function from the \textbf{spdep} R library\cite{bivand_r_2022}.
...
\subsection{Intervention Effect Estimates ($2\times4$ grid)}

% R1.5: Risk-management framing ($2\times4$ grid)
...
% --- Figure: Consolidated Intervention MSE for $2\times4$ grid ---
\begin{figure} [!ht]
\centering
\includegraphics[width=0.80\textwidth]{figures/BetaMSEresults/MSE_Estimates_2x4.png}
\caption{Intervention effect MSE for $2\times4$ cluster grid grouped by spillover effect magnitude, spatial dependence, sampling design, and spillover application type}
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - $2\times4$ Spatial Grid Setting}
\label{table:InterventionEstimates2x4}
\end{table}


\subsection{Intervention Effect Estimates ($3\times3$ grid)}
...
% --- Figure: Consolidated Intervention MSE for $3\times3$ grid ---
\begin{figure} [!ht]
\centering
\includegraphics[width=0.80\textwidth]{figures/BetaMSEresults/MSE_Estimates_3x3.png}
\caption{Intervention effect MSE for $3\times3$ cluster grid grouped by spillover effect magnitude, spatial dependence, sampling design, and spillover application type}
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - $3\times3$ Spatial Grid Setting}
\label{table:InterventionEstimates3x3}
\end{table}


\subsection{Intervention Effect Estimates ($3\times4$ grid)}

% R1.5: Risk-management framing ($3\times4$ grid); R2.M4 checkerboard paradox noted
...
% --- Figure: Consolidated Intervention MSE for $3\times4$ grid ---
\begin{figure} [!ht]
\centering
\includegraphics[width=0.80\textwidth]{figures/BetaMSEresults/MSE_Estimates_3x4.png}
\caption{Intervention effect MSE for $3\times4$ cluster grid grouped by spillover effect magnitude, spatial dependence, sampling design, and spillover application type}
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - $3\times4$ Spatial Grid Setting}
...
analysis, or when the true underlying spatial process deviates from the assumed SAR model. A natural model extension is to incorporate individual-level outcomes with a cluster-specific random effect, which would allow intra-cluster correlation (ICC) to be parameterized and estimated alongside the SAR spatial dependence and spillover coefficients. Finally, exploring optimal allocation ratios of intervention to control clusters in the presence of spillover effects and spatial dependence could provide valuable insights for maximizing statistical power or minimizing estimation error. Incorporating cost-effectiveness analysis into the design framework would also be beneficial, as different sampling strategies may have varying logistical and financial implications in real-world implementation.
...
The authors thank the editor and reviewers for their thorough and constructive feedback, which has substantially improved the manuscript.
...
\begin{enumerate}
    ...
    \item \textbf{Summarize by design.} Aggregate metrics across all SRS combinations and separately across all BSS combinations to produce the summary statistics reported in Tables~\ref{table:InterventionEstimates2x4}--\ref{table:InterventionEstimates3x4}.
