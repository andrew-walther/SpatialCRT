...
%DIF DEL /Users/ajwalther/Library/CloudStorage/OneDrive-UniversityofNorthCarolinaatChapelHill/UNC Dissertation (Lin)/Project 1 - Spillover Effects/Manuscript/Manuscript Revisions/Revision 2b/Walther_SpatialCRT_LaTeX_Revisions_V2b/Revisions_V2b.tex   Fri Jun 19 09:45:05 2026
...
In response to these challenges, a body of literature has emerged focusing on post-hoc analytical adjustments to account for spatial effects after the data have been collected. Jarvis et al. (2017) classifies these approaches into two categories: the spatial variable approach, which constructs a new covariate from distance or density metrics, and spatial modeling\cite{jarvis_spatial_2017}. Examples include measuring the straight-line distance from control participants to the nearest intervention household (Hawley et al., 2003 \& Gimnig et al., 2003)\cite{hawley_community-wide_2003}\cite{gimnig_effect_2003}, calculating outcome rates across different distance bands (Binka et al., 1998)\cite{binka_impact_1998}, or quantifying the density of treated individuals within a predefined radius (Miguel \& Kremer, 2004 \& Lenhart et al., 2008)\cite{miguel_worms_2004}\cite{lenhart_insecticidetreated_2008}. More recently, Chao et al. (2015) developed a ``potential exposure" metric to quantify the spatially heterogeneous risk surrounding an individual\cite{chao_contribution_2015}. In contrast, the spatial modeling approach is more statistically sophisticated, directly incorporating the geographic structure of the data into the random effects component of a statistical method. The method employed by Alexander et al. (2003)\cite{alexander_spatial_2003} considers a distance-decay function in the covariance matrix and an alternative by Silcocks and Kendrick (2010)\cite{silcocks_spatial_2010} using spatial weights matrices treats spatial correlation as an underlying unobserved process. Watson \& Smith (2025) propose a distinct framework in which treatment is measured as a continuous dose based on distance to the nearest intervention site, estimating a dose-response function (DRF) rather than a discrete cluster-level intervention or spillover effect\cite{watson_design_2025}.
...
\subsubsection{\DIFdelbegin \DIFdel{3x3 }\DIFdelend \DIFaddbegin \DIFadd{$3\times3$ }\DIFaddend Grid}

The \DIFdelbegin \DIFdel{3x3 }\DIFdelend \DIFaddbegin \DIFadd{$3\times3$ }\DIFaddend symmetric grid setting of 9 cells allows for %$\binom{9}{4}=\binom{9}{5}=126$ 
...
\subsubsection{\DIFdelbegin \DIFdel{3x4 }\DIFdelend \DIFaddbegin \DIFadd{$3\times4$ }\DIFaddend Grid}

The final spatial setting considered was a 3 row by 4 column (\DIFdelbegin \DIFdel{3x4}\DIFdelend \DIFaddbegin \DIFadd{$3\times4$}\DIFaddend ) grid of 12 total cells. This grid, with a balanced allocation of 6 intervention cells and 6 control cells allows for %$\binom{12}{6}=924$
...
Outcomes in the presence of spatial dependence and spillover effects can be considered via modeling with a spatial autoregressive model\cite{waller_applied_2004}\cite{fotheringham_sage_2009}\cite{ullah_handbook_1998}. \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{For the design considered in this study, the }\DIFaddend spatial autoregressive model \DIFaddbegin \DIFadd{for the cluster-level outcome }\DIFaddend has the form:

\begin{equation}\label{Equation:SARmodel}
    y\DIFdelbegin \DIFdel{_{ik} }\DIFdelend \DIFaddbegin \DIFadd{_i }\DIFaddend = \DIFaddbegin \DIFadd{\alpha + \beta x_i + \psi z_i + }\DIFaddend \rho\sum\DIFdelbegin \DIFdel{_{j \neq i} }\DIFdelend \DIFaddbegin \DIFadd{_{j \in \mathcal{N}(i)} }\DIFaddend w_{ij} y\DIFdelbegin \DIFdel{_{jk'} }\DIFdelend \DIFaddbegin \DIFadd{_j }\DIFaddend + \DIFdelbegin \DIFdel{\mathbf{x}_i \beta + }\DIFdelend \varepsilon\DIFdelbegin \DIFdel{_{ik}
}\DIFdelend \DIFaddbegin \DIFadd{_i,
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \noindent where \DIFdelbegin \DIFdel{$\mathcal{N}(i)$ denotes the set of clusters that share a Rook edge with cluster $i$ (i.e., its immediate horizontal and vertical neighbors); $w_{ij}$ are elements of the row-normalized spatial weights matrix $\mathbf{W}$, equal to $1/|\mathcal{N}(i)|$ for each Rook neighbor $j$ of }\DIFdelend \DIFaddbegin \DIFadd{$y_i$ is the cluster-level outcome for }\DIFaddend cluster $i$ \DIFdelbegin \DIFdel{and 0 otherwise (full construction in Appendix~\ref{appendix:SpatialWeights}); $i$ indexes clusters ($i = 1, \ldots, n$) and $k$ indexes individual subjects within cluster $i$}\DIFdelend \DIFaddbegin \DIFadd{(the modeled quantity)}\DIFaddend ; \DIFdelbegin \DIFdel{$k'$ is a subject index for cluster $j$, independent of $k$, so $y_{jk'}$ denotes the response of subject $k'$ in cluster $j$; $\rho$ is the spatial correlation (autoregressive)coefficient; and $\varepsilon_{ik}$ represents an i.i.d. error term for subject $k$ in cluster $i$. The matrix $\mathbf{W}$ is a cluster-level $n \times n$ adjacency matrix; the subscripts $i$ and $j$ in $w_{ij}$ refer to clusters, not individuals. This can also be expressed in matrix notation as:
}%DIFDELCMD < 
...
\DIFdel{It follows that the general form of the response for this model for the purpose of this study is
    ...
    \DIFdel{y_{ik} = \alpha + \beta x_i + \psi z_i + \rho\sum_{j \neq i} w_{ij} y_{jk'} + \varepsilon_{ik}
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $y_{ik}$ is the response for individual $k$ in cluster $i$; }\DIFdelend $\alpha$ is the baseline effect; $\beta$ is the intervention effect; $x_i \in \{0,1\}$ is the cluster-level treatment indicator (1 if cluster $i$ receives the intervention, \DIFdelbegin \DIFdel{shared by all subjects $k$ within that cluster}\DIFdelend \DIFaddbegin \DIFadd{0 otherwise}\DIFaddend ); $\psi$ is the scalar spillover parameter (in this study, spillover is determined by binary cluster adjacency, with the indicator $z_i$ capturing which clusters are exposed, so $\psi$ is a scalar multiplier rather than a function of distance); $z_i \in \{0,1\}$ is the cluster-level spillover indicator (1 if cluster $i$ is a Rook neighbor of at least one treated cluster and the spillover type permits it; 0 otherwise); \DIFaddbegin \DIFadd{$\mathcal{N}(i)$ denotes the set of clusters that share a Rook edge with cluster $i$ (i.e., its immediate horizontal and vertical neighbors); $w_{ij}$ are elements of the row-normalized spatial weights matrix $\mathbf{W}$, equal to $1/|\mathcal{N}(i)|$ for each Rook neighbor $j$ of cluster $i$ }\DIFaddend and \DIFdelbegin \DIFdel{the spatial lag $\rho\sum_{j \neq i} w_{ij} y_{jk'}$ captures background spatial dependence through the weighted average of neighboring cluster outcomes (equivalently $\rho\mathbf{Wy}$ in matrix form). Because $x_i$ and $z_i$ are cluster-level quantities constant for all subjects within }\DIFdelend \DIFaddbegin \DIFadd{0 otherwise (full construction in Appendix~\ref{appendix:SpatialWeights}); $\rho$ is the spatial autocorrelation coefficient; and $\varepsilon_i$ is the i.i.d. error term for }\DIFaddend cluster $i$\DIFdelbegin \DIFdel{, all individuals in a cluster}\DIFdelend \DIFaddbegin \DIFadd{. In matrix form,
}

\begin{equation*}
    \DIFadd{\mathbf{y} = \boldsymbol\alpha + \mathbf{X}\beta + \mathbf{Z}\psi + \rho\mathbf{W}\mathbf{y} + \boldsymbol\varepsilon.
}\end{equation*}
\noindent \DIFadd{Within any given cluster, all subjects }\DIFaddend share the same \DIFdelbegin \DIFdel{structural component of their outcome; individual responses differ only through the subject-specific error term $\varepsilon_{ik}$, so the spatial lag effectively averages over cluster-level outcomes. The spatial lag is built from $\rho$, the spatial autocorrelation parameter; $\mathbf{W}$, the row-normalized cluster-level spatial weights matrix (Appendix~\ref{appendix:SpatialWeights}); and $\mathbf{y}$, the full response vector. Finally, $\varepsilon_{ik}$ is }\DIFdelend \DIFaddbegin \DIFadd{intervention status ($x_i$), spillover exposure ($z_i$), and spatial neighborhood structure ($w_{ij}$); }\DIFaddend the \DIFaddbegin \DIFadd{only within-cluster variation in outcomes is through an independent (}\DIFaddend i.i.d.\DIFdelbegin \DIFdel{error term for subject $k$ }\DIFdelend \DIFaddbegin \DIFadd{) random error. The outcome $y_i$ is therefore modeled at the cluster level, reflecting the shared structural signal across all subjects }\DIFaddend in cluster $i$. \DIFaddbegin \DIFadd{This model specification does not parameterize intra-cluster correlation (ICC). Rather, the cluster-level outcome $y_i$ captures the structural and design signal while individual variation is treated as independent noise.
}\DIFaddend 

% R1.2: rho vs. psi distinction paragraph
It is important to clarify the conceptual distinction between two spatial quantities in this model: the spatial autocorrelation parameter $\rho$ and the spillover effect $\psi$. The parameter $\rho$ is a \emph{nuisance} parameter reflecting background spatial correlation in outcomes: nearby clusters tend to produce similar outcomes due to shared environmental, demographic, or social factors, independent of any intervention. In contrast, the spillover effect $\psi$ is a \emph{structural, design-level} parameter, specifically the coefficient on the spillover indicator $z_i$, representing the indirect (spillover) effect that an intervention in one cluster exerts on outcomes in neighboring clusters. While both quantities involve spatial proximity, $\rho$ is an intrinsic property of the data-generating process that must be accounted for in order to obtain valid inferences, whereas $\psi$ is of direct scientific interest and often a primary estimand alongside the intervention effect $\beta$. Failure to distinguish these two quantities, or to include both in the model, can produce confounded estimates of the intervention effect. In particular, when spatial dependence is high, failing to include $\rho$ in the model may cause the spillover estimate $\hat{\psi}$ to absorb background correlation, overstating the apparent indirect effect of the intervention.
...
\begin{equation}\label{Equation:SpatialDepTerm}
    \rho\mathbf{Wy} = \rho\sum\DIFdelbegin \DIFdel{_{j \neq i}}\DIFdelend \DIFaddbegin \DIFadd{_{j \in \mathcal{N}(i)}}\DIFaddend w_{ij}y\DIFdelbegin \DIFdel{_{jk'}
}\DIFdelend \DIFaddbegin \DIFadd{_j,
}\DIFaddend \end{equation}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \noindent \DIFaddend where $j$ indexes clusters \DIFdelbegin \DIFdel{, $k'$ indexes a subject in cluster $j$, }\DIFdelend and $w_{ij} = 0$ whenever clusters $i$ and $j$ are not Rook neighbors, so the sum reduces naturally to neighboring clusters only. Full definitions of indicator-based and distance-based weight construction, including row-normalization formulas, are provided in Appendix~\ref{appendix:SpatialWeights}.

\subsection{Estimation for SAR Model}

% SUPP-2: Full likelihood/log-likelihood moved to Appendix B
Maximum likelihood estimation is \DIFdelbegin \DIFdel{utilized to gather parameter estimates from the }\DIFdelend \DIFaddbegin \DIFadd{used to estimate the cluster-level }\DIFaddend spatial lag model \DIFdelbegin \DIFdel{from Section~\ref{section:SAR}}\DIFdelend \DIFaddbegin \DIFadd{in Equation~(\ref{Equation:SARmodel})}\DIFaddend \cite{ord_estimation_1975}\cite{bivand_comparing_2015}\cite{bivand_computing_2013}. The \DIFdelbegin \DIFdel{subject-level model to be estimated is
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where all terms are as defined in Section~\ref{section:SAR}. The }\DIFdelend full likelihood and log-likelihood expressions, along with details of the MLE derivation, are provided in Appendix~\ref{appendix:SARLikelihood}. In \DIFdelbegin \DIFdel{applied tasks}\DIFdelend \DIFaddbegin \DIFadd{practice}\DIFaddend , estimates are obtained via the \texttt{lagsarlm()} function from the \textbf{spdep} R library\cite{bivand_r_2022}.
...
%DIF >  --- Figure: Consolidated Intervention MSE for $2\times4$ grid ---
\begin{figure} [!ht]
\centering
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - \DIFdelbeginFL \DIFdelFL{2x4 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2\times4$ }\DIFaddendFL Spatial Grid Setting}
\label{table:InterventionEstimates2x4}
\end{table}


\subsection{Intervention Effect Estimates (\DIFdelbegin \DIFdel{3x3 }\DIFdelend \DIFaddbegin \DIFadd{$3\times3$ }\DIFaddend grid)}
...
%DIF >  --- Figure: Consolidated Intervention MSE for $3\times3$ grid ---
\begin{figure} [!ht]
\centering
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - \DIFdelbeginFL \DIFdelFL{3x3 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$3\times3$ }\DIFaddendFL Spatial Grid Setting}
\label{table:InterventionEstimates3x3}
\end{table}


\subsection{Intervention Effect Estimates (\DIFdelbegin \DIFdel{3x4 }\DIFdelend \DIFaddbegin \DIFadd{$3\times4$ }\DIFaddend grid)}

%DIF <  R1.5: Risk-management framing (3x4 grid); R2.M4 checkerboard paradox noted
%DIF >  R1.5: Risk-management framing ($3\times4$ grid); R2.M4 checkerboard paradox noted
...
%DIF >  --- Figure: Consolidated Intervention MSE for $3\times4$ grid ---
\begin{figure} [!ht]
\centering
...
\begin{tabular}{l c c ccc ccc}
    ...
    \bottomrule
\end{tabular}}
\caption{SRS vs. BSS: Intervention Effect Error Estimates - \DIFdelbeginFL \DIFdelFL{3x4 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$3\times4$ }\DIFaddendFL Spatial Grid Setting}
...
The authors thank the \DIFdelbegin \DIFdel{Editor and both Reviewers }\DIFdelend \DIFaddbegin \DIFadd{editor and reviewers }\DIFaddend for their thorough and constructive feedback, which has substantially improved the manuscript.
...
\DIFdelbegin \begin{displaymath}
    \DIFdel{W_{ij} = \frac{\tilde{w}_{ij}}{\displaystyle\sum_{j \neq i} \tilde{w}_{ij}}
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
...
where $d_{ij}$ is the distance between clusters $i$ and $j$ and $d_{\max}$ is the maximum distance threshold for neighborhood inclusion. These weights are also row-normalized via
\DIFdelbegin \begin{displaymath}
    \DIFdel{W_{ij} = \frac{\tilde{w}_{ij}}{\displaystyle\sum_{j \neq i}\tilde{w}_{ij}}.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
...
\DIFdelbegin \DIFdel{(subject subscript $k$ suppressed for clarity)
}\begin{displaymath}
...
\DIFdelbegin \begin{displaymath}
    \DIFdel{L\!\left(\alpha,\beta,\psi,\rho,\sigma^2\right)
...
\DIFdelbegin \begin{align*}
    \DIFdel{\log L\!\left(\alpha,\beta,\psi,\rho,\sigma^2\right)
    }&\DIFdel{= \log|\mathbf{I} - \rho\mathbf{W}|
       - \frac{n}{2}\log\!\left(2\pi\sigma^2\right) \notag }\\
    &\DIFdel{\quad - \frac{1}{2\sigma^2}
        \sum_{i=1}^{n}
...
\begin{enumerate}
    ...
    \item \textbf{Summarize by design.} Aggregate metrics across all SRS combinations and separately across all BSS combinations to produce the summary statistics reported in Tables~\DIFdelbegin \DIFdel{1--3}\DIFdelend \DIFaddbegin \DIFadd{\ref{table:InterventionEstimates2x4}--\ref{table:InterventionEstimates3x4}}\DIFaddend .
