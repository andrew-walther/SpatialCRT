...
%\usepackage{float}
\usepackage{graphicx}
\graphicspath{{../Walther_SpatialCRT_LaTeX_Revisions_V2b/}}
\usepackage{caption}
...
In response to these challenges, a body of literature has emerged focusing on post-hoc analytical adjustments to account for spatial effects after the data have been collected. Jarvis et al. (2017) classifies these approaches into two categories: the spatial variable approach and spatial modeling\cite{jarvis_spatial_2017}. The more common spatial variable approach constructs a covariate from distance or density metrics---for example, the distance from control participants to the nearest intervention household, outcome rates across distance bands, the density of treated individuals within a predefined radius, or a ``potential exposure" surface\cite{hawley_community-wide_2003}\cite{gimnig_effect_2003}\cite{binka_impact_1998}\cite{miguel_worms_2004}\cite{lenhart_insecticidetreated_2008}\cite{chao_contribution_2015}. The spatial modeling approach instead incorporates the geographic structure of the data directly into the covariance or random-effects component of a statistical model\cite{alexander_spatial_2003}\cite{silcocks_spatial_2010}. A distinct framework randomly assigns intervention areas and treats ``treatment" as a continuous dose based on distance to the nearest intervention site, estimating a dose-response function (DRF) rather than a cluster-level intervention or spillover effect\cite{watson_design_2025}.
...
% =============================================================================
\section{Intervention Effect Estimate Tables}\label{appendix:ResultTables}
% (Short version: Tables 1--3 relocated from Sections 3.4--3.6 to reduce length)
% =============================================================================

The complete numerical results summarized in the Simulation Study (Section~\ref{section:Results}) are tabulated below for each spatial grid geometry.
