...
- Expand spatial simulation w/ spillover estimation to a 3x3 and/or 4x4 grid (beyond previous 4x2 grid) to understand estimation efficiency on larger spaces.
...
Num.Combos <- 126 # Number of trt combos to iterate over
points_per_iteration <- 180  # Number of points per dataset (20 per district)
grid_width <- 300 # Set dimensions of grid for simulated data
grid_height <- 300
cell_length <- 100

# Define response model constants (true values) (iterate through combos of psi/rho/TrtSpill)
alpha <- 0.2 # intercept/baseline effecgt
beta <- 1 # consider a range of intervention effects (start with Null case -> 1)
psi <- 0.8 # maximum spillover effect (binary) (options: 0.5, 0.6, 0.7, 0.8)
rho <- 0.0 # spatial correlation parameter (options: 0.00, 0.01)
sd <- 0.1 # set standard deviation (for epsilon) -> change this to reduce "noise"/more consistent estimates
```

- For 3x3 grid with 4 or 5 control/intervention and 4 or 5 of the other (likely choose 4 intervention), there are 9 choose 4 (or 9 choose 5) = 126 combinations of how to select these grid setups.

- For a 4x4 grid (16 cells) with 8 intervention and 8 control districts, there are 9 choose 8 = 12870 different combinations of how to choose the grid setup (this seems impractical). Note: 16 choose 4 = 1820 combinations.
...
for (i in 1:ncol(combinations)) {
  ...
  results[i, 2:10] <- treatment_assignment
}

# Rename the columns
colnames(results) <- c('Combination', paste('Cell', 1:9))
...
neighbor_matrix <- matrix(0, nrow = 9, ncol = 9)

# Define the neighbor relationships (cells are indexed from 1 to 8)
rook_neighbors <- list(
  `1` = c(2, 4), # Cell 1 neighbors
  `2` = c(1, 3, 5), # Cell 2 neighbors
  `3` = c(2, 6), # Cell 3 neighbors
  `4` = c(1, 5, 7), # Cell 4 neighbors
  `5` = c(2, 4, 6, 8), # Cell 5 neighbors
  `6` = c(3, 5, 9), # Cell 6 neighbors
  `7` = c(4, 8), # Cell 7 neighbors
  `8` = c(5, 7, 9), # Cell 8 neighbors
  `9` = c(6, 8) # Cell 9 neighbors
)
...
# row 1
district_1 <- st_polygon(list(matrix(c(0, 200, 100, 200, 100, 300, 0, 300, 0, 200), 
                                     ncol = 2, byrow = TRUE))) # Top-left
district_2 <- st_polygon(list(matrix(c(100, 200, 200, 200, 200, 300, 100, 300, 100, 200), 
                                     ncol = 2, byrow = TRUE))) # Top-center
district_3 <- st_polygon(list(matrix(c(200, 200, 300, 200, 300, 300, 200, 300, 200, 200), 
                                     ncol = 2, byrow = TRUE))) # Top-right
# row 2
district_4 <- st_polygon(list(matrix(c(0, 100, 100, 100, 100, 200, 0, 200, 0, 100), 
                                     ncol = 2, byrow = TRUE))) # Middle-left
district_5 <- st_polygon(list(matrix(c(100, 100, 200, 100, 200, 200, 100, 200, 100, 100), 
                                     ncol = 2, byrow = TRUE))) # Center
district_6 <- st_polygon(list(matrix(c(200, 100, 300, 100, 300, 200, 200, 200, 200, 100), 
                                     ncol = 2, byrow = TRUE))) # Middle-right
# row 3
district_7 <- st_polygon(list(matrix(c(0, 0, 100, 0, 100, 100, 0, 100, 0, 0), 
                                     ncol = 2, byrow = TRUE))) # Bottom-left
district_8 <- st_polygon(list(matrix(c(100, 0, 200, 0, 200, 100, 100, 100, 100, 0), 
                                     ncol = 2, byrow = TRUE))) # Bottom-center
district_9 <- st_polygon(list(matrix(c(200, 0, 300, 0, 300, 100, 200, 100, 200, 0), 
                                     ncol = 2, byrow = TRUE))) # Bottom-right


# Combine all districts into one list
districts_list <- list(district_1, district_2, district_3, district_4, district_5, district_6, district_7, district_8, district_9)
districts_sf <- st_sfc(districts_list, crs = NA_crs_) # this is the spatial object

### Manually Adjusted Centroids for 3x3 grid ###
district_names <- c("1", "2", "3", "4", "5", "6", "7", "8", "9")
label_positions <- data.frame(
  district = district_names,
  x = c(50, 150, 250, 50, 150, 250, 50, 150, 250),  # X-coordinates
  y = c(250, 250, 250, 150, 150, 150, 50, 50, 50)   # Y-coordinates
)
...
ggplot() +
  ...
  theme_minimal() +
  ggtitle("Spatial region with district labels (1-9)") +
  theme(
...
# Generate all 126 treatment assignments
districts <- 1:9
...
- Combos that satisfy "block stratification": 28, 30, 35, 51, 77, 106
...
###########################################################################
### Apply functions to all 70 assignments ###
...
- Combo 77 is only setup where the all control cells & intervention cells are not adjacent (rook neighbors) with other cells of the same treatment type. However, if we relax the rook neighbor restriction on the control cells (controls can be adjacent to controls), we also get that combos 28, 30, 35, 51, 106 satisfy the block stratification condition as well!
...
neighbor_matrix <- matrix(0, nrow = 9, ncol = 9, dimnames = list(1:9, 1:9))
...
assign_district <- function(x, y) {
  if (x >= 0 & x < 100 & y >= 200 & y < 300) return(1)  # District 1
  if (x >= 100 & x < 200 & y >= 200 & y < 300) return(2) # District 2
  if (x >= 200 & x < 300 & y >= 200 & y < 300) return(3) # District 3
  if (x >= 0 & x < 100 & y >= 100 & y < 200) return(4)   # District 4
  if (x >= 100 & x < 200 & y >= 100 & y < 200) return(5) # District 5
  if (x >= 200 & x < 300 & y >= 100 & y < 200) return(6) # District 6
  if (x >= 0 & x < 100 & y >= 0 & y < 100) return(7)     # District 7
  if (x >= 100 & x < 200 & y >= 0 & y < 100) return(8)   # District 8
  if (x >= 200 & x < 300 & y >= 0 & y < 100) return(9)   # District 9
}
```
...
for (combo_index in 1:length(combinations)) {
  ...
  for (dataset_num in 1:N) {
    ...
    A <- I - rho * W # (I - rho*W)
    points$response <- solve(A, linear_response) # (I - rho W)^(-1) * (a + XB + e) - simultaneous response (solve system of equations)
    ####################################################################################################
...
compute_metrics <- function(beta_estimates, true_value) {
  results <- as.data.frame(t(apply(beta_estimates, 2, function(estimates) {
    ...
    mse <- mean((estimates - true_value)^2) # MSE = bias^2 + var
...
write.csv(beta.mse, "../results/beta_mse/beta_mse_results_TrtNoSpill_3x3_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(alpha.mse, "../results/alpha_mse/alpha_mse_results_TrtNoSpill_3x3_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(psi.mse, "../results/psi_mse/psi_mse_results_TrtNoSpill_3x3_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(rho.mse, "../results/rho_mse/rho_mse_results_TrtNoSpill_3x3_A02_B1_P08_R00.csv", row.names = TRUE)
