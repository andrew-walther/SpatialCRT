...
rho <- 0.00 # spatial correlation parameter (options: 0.00, 0.01)
...
for (combo_index in 1:length(combinations)) {
  ...
  for (dataset_num in 1:N) {
    ...
    W <- as.matrix(neighbor_matrix) # do we need to row-standardize Sum(w_ij) = 1 this?
    
    # W_listw <- mat2listw(W_raw, style = "W") # Convert raw adjacency matrix to listw with row-standardization (style="W")
    # W_standardized <- listw2mat(W_listw) # If you need matrix format back (or direct matrix multiplication)
...
write.csv(beta.mse, "../results/beta_mse/beta_mse_results_TrtNoSpill_2x4_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(alpha.mse, "../results/alpha_mse/alpha_mse_results_TrtNoSpill_2x4_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(psi.mse, "../results/psi_mse/psi_mse_results_TrtNoSpill_2x4_A02_B1_P08_R00.csv", row.names = TRUE)
write.csv(rho.mse, "../results/rho_mse/rho_mse_results_TrtNoSpill_2x4_A02_B1_P08_R00.csv", row.names = TRUE)
