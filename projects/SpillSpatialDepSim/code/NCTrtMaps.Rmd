...
judicial_assignments <- read.csv("../data/NC_Judicial_District_Assignments.csv") %>%
  clean_names() %>%
...
```{r, warning=FALSE}
### Generate just one random map of intervention assignments ### (intermediate step!)
...
# =================================================================
# MAP 1: Block Stratified Sampling Assignment
# =================================================================
map_deterministic <- ggplot() +
  ...
  geom_text(
    ...
    aes(x = lon, y = lat, label = district), color = "white", size = 2.5, fontface = "bold"
  ) +
  ...
  labs(
    #title = "Map 1: Block Stratified Sampling Assignment",
    #subtitle = "Blocking of Treatment & Control"
  ) +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
# =================================================================
# MAP 2: Simple Random Sampling Assignment
# =================================================================
map_random <- ggplot() +
  ...
  scale_color_manual(values = division_border_colors_safe, name = "Judicial Division") +
  labs(
    #title = "Map 2: Simple Random Sampling Assignment",
...
```
```{r}
### Maps w/o highlighting by division (labels added) ###
# --- 1. Deterministic Assignment (Hard-Coded Assignment) ---
# Goal: Districts 1-16 = Treatment, All Others = Control (as defined by your list)
...
# Explicitly restore the 'sf' class after the randomization (Crucial Fix)
if (!inherits(nc_districts_sampled, "sf")) {
  nc_districts_sampled <- sf::st_set_geometry(nc_districts_sampled, nc_districts_sampled$geometry)
}
...
# --- 4. Prepare Division Label Data (New Step) ---

division_centers <- nc_divisions %>%
  sf::st_centroid() %>%
  dplyr::mutate(
    Division_Name = paste("Division", division),
    lon = sf::st_coordinates(.)[, "X"],
    lat = sf::st_coordinates(.)[, "Y"]
  ) %>%
  sf::st_drop_geometry()

# Manually adjust label positions for visual clarity
# division 1
division_centers[division_centers$division == "1", "lat"] <- division_centers[division_centers$division == "1", "lat"] + 0.25
division_centers[division_centers$division == "1", "lon"] <- division_centers[division_centers$division == "1", "lon"] + 0.30
# division 2
division_centers[division_centers$division == "2", "lat"] <- division_centers[division_centers$division == "2", "lat"] + 0.80
division_centers[division_centers$division == "2", "lon"] <- division_centers[division_centers$division == "2", "lon"] - 0.00
# division 3
division_centers[division_centers$division == "3", "lat"] <- division_centers[division_centers$division == "3", "lat"] + 0.18
division_centers[division_centers$division == "3", "lon"] <- division_centers[division_centers$division == "3", "lon"] - 0.15
# division 4
division_centers[division_centers$division == "4", "lat"] <- division_centers[division_centers$division == "4", "lat"] + 0.15
division_centers[division_centers$division == "4", "lon"] <- division_centers[division_centers$division == "4", "lon"] + 0.35


# --- 5. Define Color Scales (Simplified) ---
intervention_colors <- c("Treatment" = "red3", "Control" = "dodgerblue3") 
...
# =================================================================
map_deterministic <- ggplot() +
  # 1. Draw Districts (FILL by Intervention)
  geom_sf(data = nc_map_final_deterministic_labeled, 
          aes(fill = intervention), color = "gray60", linewidth = 0.2) +
  
  # 2. Draw Divisions (SIMPLE GRAY BORDER) - No AES mapping to color
  geom_sf(data = nc_divisions, color = "gray20", fill = NA, linewidth = 1.0) + 
  
  # 3. Add Floating Division Labels
  geom_text(
    data = division_centers,
    aes(x = lon, y = lat, label = Division_Name),
    color = "gold", 
    size = 3.5, 
  ...
  scale_fill_manual(
    ...
    labels = c("Treatment" = "Intervention", "Control" = "Control")
    ) +  
  # 6. Remove scale_color_manual (because division borders are not mapped to color aesthetic)
  labs(
    #title = "Map 1: Block Stratified Sampling Assignment",
    #subtitle = "Blocking of Treatment & Control"
  ) +
